...
class UIHierarchyAccessibilityService : AccessibilityService() {
    ...
    companion object {
        ...
        private const val OUTPUT_DIR = "/storage/emulated/0/Controller"
    ...
    private fun serializeNode(node: AccessibilityNodeInfo): JSONObject {
        ...
        obj.put("boundsInScreen", rectToJson(boundsScreen))

        val boundsParent = Rect()
        node.getBoundsInParent(boundsParent)
        obj.put("boundsInParent", rectToJson(boundsParent))
    ...
    private fun appendToFile(content: String) {
        try {
            ...
            BufferedWriter(FileWriter(File(dir, OUTPUT_FILE), true)).use { writer ->
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        // Periodic capture handled by timer; event-driven capture not needed
    }
